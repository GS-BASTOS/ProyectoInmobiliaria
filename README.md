...
CRM interno para la gestión de clientes interesados, inmuebles, visitas
y proveedores. Desarrollado con Spring Boot 3 + Thymeleaf + MySQL.
...
-   Estados de interés por colores (sin contacto, visita programada,
    oferta, etc.)
...
-   Catálogo interno de inmuebles con código Macro, tipo, dirección y
    municipio
-   Estados: disponible, pre-vendido, vendido
-   Subida de imágenes via Cloudinary
-   Catálogo público para clientes externos
-   Cascada automática: al vender un inmueble, descarta las
    interacciones de otros clientes
...
src/ ├── main/ │ ├── java/com/inmobiliaria/app/ │ │ ├── domain/ \#
Entidades JPA │ │ ├── repo/ \# Repositorios Spring Data │ │ ├── service/
\# Lógica de negocio │ │ └── web/ \# Controladores + DTOs │ └──
resources/ │ ├── templates/ \# Plantillas Thymeleaf │ └── static/ │ ├──
css/ \# Estilos por página │ └── js/ \# JavaScript por página

------------------------------------------------------------------------

## 🔐 Seguridad

La aplicación usa Spring Security. El acceso al panel de gestión
requiere autenticación. La web pública (`/catalogo`, `/contacto`) es
accesible sin login.
